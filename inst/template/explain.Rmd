...
```{r rec-core}
rec_core <- get_core_recipe_tb(tbl)
```
...
```{r rec-log}
rec_log <- get_log_recipe_tb(rec_core)

```

```{r}
rec_norm <- get_normalize_recipe_tb(rec_core)
```

```{r}
rec_pop_100k <- get_pop_100k_recipe_tb(rec_core)
```
...
wf <-
  ...
  workflows::add_recipe_tb(rec_core_prep) 
```
