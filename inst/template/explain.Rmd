...
```{r}
tbl_rec <-
  recipe(formula = NULL, x = tbl) |>
  step_filter_missing(all_predictors(), threshold = 0.9) |> 
  step_rm(c_cdr, c_newinc_100k, e_inc_num, e_mort_100k, e_pop_num, new_labconf) |>
  update_role(country_code, new_role = "id") |>
  update_role(pop_urban_perc, new_role = "no_norm") |>
  update_role(pop_density, new_role = "no_norm") |>
  update_role(
    dplyr::all_of(c("e_inc_100k", "e_inc_num", "c_newinc", "pop_total", "gdp")),  
    new_role = "impute"
  ) |>
  step_mutate(who_dx_gap = (e_inc_num - c_newinc) / e_inc_num) |> 
  step_impute_median(has_role("impute")) |> 
  step_impute_median(budget_oth, cf_tot_gf, cf_tot_grnt) |>
  step_rm(e_inc_100k, e_inc_num, c_newinc) |>
