...
generalise these methods into a broader R package framework. This creates the
need for a project structure that both allows for a high-level of
...
running `renv::restore()`. If this is not your first time opening this project,
call `renv::restore()` manually.
...
Data for the project are stored in a directory called `find.dxgap.data`. Please, call `set_data_dir()` to create and populate such folder.
...
Call, `render_report(year = 2019)` to render a report for given year. Create new templates and render them using the `template_path` parameter.
