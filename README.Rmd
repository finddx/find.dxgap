...
Data for the project are stored in a directory called `findtb.data`. Please, call `set_findtb_data_dir()` to create and populate such folder.
...
Call, `findtb_render_report(year = 2019)` to render a report for given year. Create new templates and render them using the `template_path` parameter.
