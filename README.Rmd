...
The following env. variable needs to be set to establish connection between `find.dxgap` and `find.dxgap.data`:

```
DXGAP_DATADIR = "/<path_to>/find.dxgap.data"
```

Please call, `usethis::edit_r_environ(scope = "user")` and place it in your .Renviron file.
