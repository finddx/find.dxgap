get_core_recipe <- function(tbl) {
  recipes::recipe(formula = who_dx_gap ~ ., x = tbl) |>
    ...
    recipes::step_filter_missing(all_predictors(), threshold = 0.9) |>
    recipes::update_role(
      dplyr::all_of(
    ...
    recipes::step_impute_median(has_role("impute_w_median")) |>
    recipes::step_mutate(pop_100k = pop_total / 1e5) |>
    recipes::step_rm(has_role("collinear")) |>
    recipes::update_role(
    ...
    recipes::step_zv(all_numeric_predictors())
}
